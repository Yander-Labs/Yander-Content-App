...
def main():
    print("=" * 60)
    print("CREATING EMPLOYEE RETENTION VIDEO SCRIPT")
    print("=" * 60)
    ...
    script_path = f"output/scripts/script_employee_quit_{timestamp}.json"

    os.makedirs(os.path.dirname(script_path), exist_ok=True)
    with open(script_path, 'w', encoding='utf-8') as f:
        json.dump(EMPLOYEE_QUIT_SCRIPT, f, indent=2)
    ...
    for section in EMPLOYEE_QUIT_SCRIPT["main_sections"]:
        ...
        status = "OK" if char_count < 1000 else "LONG" if char_count < 1800 else "TOO LONG"
        print(f"    - {section['section_title'][:35]:<35}: {char_count:>4} chars [{status}]")

    # Create Notion entry
    print(f"\n[2/2] Creating Notion entry...")

    idea = {
        "title": EMPLOYEE_QUIT_SCRIPT["title"],
        "hook": EMPLOYEE_QUIT_SCRIPT["hook"]["script"],
        "key_points": [section["section_title"] for section in EMPLOYEE_QUIT_SCRIPT["main_sections"]]
    }

    page_id = notion_agent.create_video_entry(idea, EMPLOYEE_QUIT_SCRIPT)
    ...
    print("=" * 60)
    print(f"\nTitle: {EMPLOYEE_QUIT_SCRIPT['title']}")
    print(f"Duration: ~{EMPLOYEE_QUIT_SCRIPT['estimated_length_minutes']} minutes")
    print(f"Sections: {len(EMPLOYEE_QUIT_SCRIPT['main_sections'])}")
